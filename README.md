...
5. Aop（切面）：
   > 参考目录com.springboot.framework.aop下AdviceAop类。
6. Transactional（事务）：
   > 参考目录com.springboot.framework.service.impl下AdminServiceImpl类。
7. Scheduled（定时器）：
   > 参考目录com.springboot.framework.service.impl下AdminServiceImpl类。
8. Interceptor（拦截器）
   > 参考目录com.springboot.framework.interceptor下AccessControlInterceptor类。
